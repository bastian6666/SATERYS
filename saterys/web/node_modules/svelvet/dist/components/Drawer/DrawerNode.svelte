...
let x = 0;
let y = 0;
export const createNodeProps = (x2, y2, edgeProps, anchorProps) => {
  const snappedPosition = getSnappedPosition(x2, y2);
  x2 = snappedPosition.x;
  y2 = snappedPosition.y;
  const nodeProps2 = { x: x2, y: y2 };
  const nodePropNames = [
    "bgColor",
    "borderColor",
    "label",
