...
- **NEW:** Full vector data support (GeoJSON, SHP, GPKG) with interactive map overlays
- **NEW:** Create and visualize points, lines, and polygons directly on the map
...
### 🤖 **AI-Powered Workflow Builder** (NEW!)
- Built-in AI chat assistant powered by OpenAI GPT models
- Natural language workflow creation and modification
- Intelligent node suggestions and connections
- Context-aware help and guidance
- Auto-generate complete pipelines from descriptions
...
| 📍 **vector.input** | Load vector data (GeoJSON/SHP/GPKG) | Vector data ingestion |
| ✏️ **vector.create** | Create points, lines, polygons | Feature creation |
...
# Get vector GeoJSON
GET /preview/vector/{id}

# AI Chat
POST /llm/chat
{
  "message": "Create a workflow to calculate NDVI",
  "context": {"nodes": [...], "edges": [...]}
}
```
...
SATERYS includes an AI-powered chat assistant to help you build workflows:
...
3. **Open the AI Chat panel** by clicking the "AI Chat" button in the top toolbar

4. **Ask the AI to help you:**
   - "Create a workflow to calculate NDVI from a raster"
   - "Add a vector polygon to mark my study area"
   - "How do I calculate water indices?"
   - "Build a complete NDVI analysis pipeline"

The AI assistant has full context of your workflow and can:
- Create new nodes with proper configuration
- Connect nodes automatically
- Modify existing nodes
- Answer questions about geospatial analysis
- Generate complete multi-node workflows

**Note:** The AI chat requires an active OpenAI API key. The default model is `gpt-4o-mini` for cost-effectiveness. You can change this with:
```bash
export SATERYS_LLM_MODEL="gpt-4"
```
...
# AI Chat (OpenAI)
export OPENAI_API_KEY=sk-your-key-here
export SATERYS_LLM_MODEL=gpt-4o-mini  # or gpt-4, gpt-3.5-turbo, etc.
```
